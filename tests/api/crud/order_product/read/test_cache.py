...
async def test_cache_op(
    ...
    response = await client.get(
        ''.join([URLS['crud']['order_product']['read'], op_id]),
        headers={'Authorization': f'Bearer {access_token}'},
    )

    assert 'op' in response.json()
    assert 'cached_op' not in response.json()
    ...
    assert 'op' not in response.json()
    assert 'cached_op' in response.json()
    assert response.status_code == expected_status
