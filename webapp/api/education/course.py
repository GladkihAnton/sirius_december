...
async def get_all_courses_endpoint(
    ...
    page_size: int = Query(
        10, ge=1, le=100, description='Количество курсов на странице'),
...
async def get_course_subscribe_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Ошибка при подписке') from e
