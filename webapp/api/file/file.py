...
from webapp.crud.file import (create_file, delete_file, get_file_by_id,
                              get_files_by_lesson_id)
...
async def upload_file_endpoint(
    ...
    if current_user['role'] not in ['admin', 'teacher']:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail='Нет доступа для выполнения этой операции')
    try:
        ...
        # возвращаем указатель в начало файла после его чтения
        file.file.seek(0)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Ошибка хранилища: {str(e)}')
...
async def get_files_endpoint(
    ...
    page_size: int = Query(
        10, ge=1, le=100, description='Количество файлов на странице'),
    current_user: JwtTokenT = Depends(jwt_auth.get_current_user),
    session: AsyncSession = Depends(get_session),
    redis: Redis = Depends(get_redis),
):
    cache_key = get_files_by_lesson_cache_key(
        course_id, lesson_id, page, page_size)
    ...
    try:
        ...
        if not result:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail='Файлы не найдены')
...
async def download_file_endpoint(
    ...
    try:
        ...
        response = minio_client.get_object(
            f'course-{course_id}-lesson-{lesson_id}', file_path)
